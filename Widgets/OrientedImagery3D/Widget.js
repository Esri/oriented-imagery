...
define([
    ...
    "https://oi1.img.arcgis.com/api/v2.4/main.js",
    "dijit/form/Select",
    "dijit/form/Button",
    "dijit/form/CheckBox",
    "dijit/form/TextBox"
    ...
    function (
        ...
        var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            IdentityManager.registerOAuthInfos([this.oauthInfo]);
                ...
                document.getElementById('portal-login-button').addEventListener('click', lang.hitch(this, this.callEnterpriseLogin));
                ...
                document.getElementById('portal-url').addEventListener('change', lang.hitch(this, this.checkPortalVersion));
        
            },

            checkPortalVersion: function () {
                var def = new Deferred();
                var portalUrl = document.getElementById('portal-url').value;
                if (portalUrl) {
                    portalUrl = portalUrl[portalUrl.length - 1] === '/' ? portalUrl : portalUrl + '/';
                    esriRequest(portalUrl + 'sharing/rest', {
                        query: {
                            f: "json"
                        },
                        "responseType": "json"
                        ...
                        document.getElementById('portal-login-button').disabled = false;
    
                        if (response.data.currentVersion >= 8.2) {
                            document.getElementById('portal-client').value = 'orientedimagery';
                            this.olderPortal = false;  //flag whether it is an older portal below 10.8.1
                        } else {
                            // document.getElementById('portal-error').style.display = '';
                            // document.getElementById('portal-login-button').disabled = true;
                            this.olderPortal = true;
                        }
                        def.resolve();
                ...
                orientedImagery.on("load", lang.hitch(this, function (loaded) {
                    if (loaded) {
                        ...
                        this.orientedViewer.on("imageToGroundPoint", lang.hitch(this, function (pointJson) {
                            this.sceneView.map.ground.queryElevation(new Point(pointJson)).then(lang.hitch(this, function (res) {
                            pointJson.z += res.geometry.z;
                ...
                if (this.widgetOpen && this.selectOIC.value !== "select") {
                    if (this.mapViewState) {
                        ...
                        this.sceneView.hitTest(evt.screenPoint).then(lang.hitch(this, function (response) {
                            ...
                            if (this.distanceSelector && this.currentImageCameraLocation && this.imageDialog.open) {
            ...
            searchImages: function (point, objectId) {
                ...
                if (this.oiApiLoaded) {
                    //point.z = 0;
                    this.sceneView.map.ground.queryElevation(point).then(lang.hitch(this, function (res) {
                        ...
                        if (this.mapViewState) {
                            ...
                            this.orientedViewer.searchImages(pointJSON, url, { maxDistance: 1000, camera: this.sceneView.camera, mapSize: { w: this.sceneView.width, h: this.sceneView.height }, extent: this.sceneView.extent.toJSON(), objectId: objectId || null });
                    }));
                }
            },
            drawPointAndPolygons: function (response) {
                ...
                if (!response.error) {
                    ...
                    var multiPoint = this.createMultiPoint({geometries: response.imageSourcePoints.concat(response.point), type: "point"});
                ...
                this.sceneView.map.ground.queryElevation(multiPoint).then(lang.hitch(this, function (res) {
                    for (var cv = 0; cv < res.geometry.points.length; cv++) {
                        if (cv < response.imageSourcePoints.length) {
                            response.imageSourcePoints[cv].z += res.geometry.points[cv][2];
                        } else if (cv === response.imageSourcePoints.length) {
                            response.point.z += res.geometry.points[cv][2];
                        } else {
                            for (var fm = 0; fm < response.coverageFrustums.length; fm++) {
                                for (var dm = 2; dm < response.coverageFrustums[fm].vertexAttributes.position.length; dm = dm + 3) {
                                    response.coverageFrustums[fm].vertexAttributes.position[dm] += res.geometry.points[cv][2];
                                    cv++;
                                }
                            }
                        }
            ...
            updateCoveragePolygon: function (imageProperties) {
                if (imageProperties.coverageFrustum.vertexAttributes) {
                    ...
                    this.sceneView.map.ground.queryElevation(multiPoint).then(lang.hitch(this, function (result) {
                        var cb = 0;
                    for (var bm = 2; bm < imageProperties.coverageFrustum.vertexAttributes.position.length; bm = bm + 3) {
                        imageProperties.coverageFrustum.vertexAttributes.position[bm] += result.geometry.points[cb][2];
                        cb++;
                    }
                    for (var v = this.graphicsLayer.graphics.items.length - 1; v >= 0; v--) {
            ...
            showHideInput: function (value) {
                ...
                domStyle.set(this.addOICBtn, 'display', '');
                if (value === "url") {
                    ...
                    if (this.agolOICList.value)
                        this.addOICBtn.disabled = false;
                    this.callAgolLogin();
                } else if (value === 'enterprise') {    //<!--enterprise-->
                    this.checkPortalVersion().then(lang.hitch(this, function (err) {
                        if (!err) {
                            domStyle.set(this.urlInput, "display", "none");
                            domStyle.set(this.fileInput, "display", "none");
                            domStyle.set(this.sampleInput, "display", "none");
                            ...
                            if (!this.portalLogged) {
                                domStyle.set('portal-login', 'display', 'block');
                                domStyle.set('agol-portal-div', 'display', 'none');
                                domStyle.set(this.addOICBtn, 'display', 'none');
                            } else {
                                domStyle.set('portal-login', 'display', 'none');
                                domStyle.set('agol-portal-div', 'display', 'block');
                                domStyle.set(this.addOICBtn, 'display', 'block');
                            }
    
                            if (document.getElementById("agolOICList").value) {
                                this.addOICBtn.disabled = false;
                            }
    
                            document.getElementById('portal-url').value = sessionStorage.getItem('enterprisePortal') ? sessionStorage.getItem('enterprisePortal') : '';
                            document.getElementById('portal-client').value = sessionStorage.getItem('enterpriseClient') ? sessionStorage.getItem('enterpriseClient') : '';
                            //document.getElementById('portal-login-button').click();
                            this.callEnterpriseLogin();
                        }
                    }));
                } else {
                    domStyle.set(this.urlInput, "display", "none");
            ...
            callAgolLogin: function () {   //oauth
                ...
                IdentityManager.registerOAuthInfos([this.oauthInfo]);
                IdentityManager.checkSignInStatus(this.oauthInfo.portalUrl + '/sharing').then(lang.hitch(this, function (user) {  //enterprise
                    this.getOICFromUserAcc(user);
                })).catch(lang.hitch(this, function () {
                    // html.set("itemNotify", "");
                    // this.hideLoading();
                    IdentityManager.getCredential(this.oauthInfo.portalUrl + '/sharing', { oAuthPopupConfirmation: false }).then(lang.hitch(this, function (user) {
                        this.getOICFromUserAcc(user);
            ...
            callEnterpriseLogin: function () {   //enterprise
                this.showLoading();
                this.oauthInfo = null;
                if (document.getElementById('portal-url').value && this.olderPortal) {
                    this.oauthInfo = {};
                    this.oauthInfo.portalUrl = document.getElementById('portal-url').value;
                    this.getOICFromUserAcc({server: document.getElementById('portal-url').value});
                }
                else if (document.getElementById('portal-client').value && document.getElementById('portal-url').value) {
                    this.oauthInfo = new OAuthInfo({
                        appId: 'orientedimagery', //document.getElementById('portal-client').value,
                        portalUrl: document.getElementById('portal-url').value,
                        popup: true
                    });
                    IdentityManager.registerOAuthInfos([this.oauthInfo]);
                } else {
                ...
                if (this.oauthInfo) {
                    IdentityManager.checkSignInStatus(this.oauthInfo.portalUrl + '/sharing').then(lang.hitch(this, function (user) {  //enterprise
                        ...
                        domStyle.set('agol-portal-div', 'display', 'block');
                        domStyle.set(this.addOICBtn, 'display', 'block');
                        sessionStorage.setItem('enterprisePortal', document.getElementById('portal-url').value);
                        sessionStorage.setItem('enterpriseClient', document.getElementById('portal-client').value);
                        this.portalLogged = true;
                        this.getOICFromUserAcc(user);
                    })).catch(lang.hitch(this, function (error) {
                        IdentityManager.getCredential(this.oauthInfo.portalUrl + '/sharing', { oAuthPopupConfirmation: false }).then(lang.hitch(this, function (user) {
                            domStyle.set('portal-login', 'display', 'none');
            ...
            getOICFromUserAcc: function (user) {
                this.showLoading();
                var portal = new arcgisPortal(user.server);
                portal.authMode = "immediate";
                if (this.olderPortal && !this.portalLogged) {
                    domStyle.set(this.addOICDialog, "display", 'none');
                } else {
                    domStyle.set(this.addOICDialog, "display", '');
                }
                portal.load().then(lang.hitch(this, function (user) {
                    if (this.olderPortal) {
                        this.portalLogged = true;
                    }
                    domStyle.set(this.addOICDialog, "display", '');
            ...
            getOICFromFolders: function (loggedInUser) {
                var requestUrl = this.oauthInfo.portalUrl[this.oauthInfo.portalUrl.length - 1] === '/' ? this.oauthInfo.portalUrl : this.oauthInfo.portalUrl + '/';
                esriRequest(loggedInUser.userContentUrl, {
                    ...
                    for (var a = 0; a < userContent.items.length; a++) {
                        if (userContent.items[a].type === "Oriented Imagery Catalog") {
                            this.userContentInfo.myFolders[userContent.currentFolder || "default"].items.push({ name: userContent.items[a].title, url: requestUrl + "sharing/rest/content/items/" + userContent.items[a].id });
                        }
                    }
            ...
            getOICFromGroup: function (value) {
                ...
                esriRequest(requestUrl + "sharing/rest/content/groups/" + id, {
                    query: {
                        f: "json"
                    },
                    responseType: "json"
                        ...
                        for (var a = 0; a < response.items.length; a++) {
                            if (response.items[a].type === "Oriented Imagery Catalog") {
                                if (this.userContentInfo.myGroups[value]) { //groupchange
                                    this.userContentInfo.myGroups[value].items.push({ name: response.items[a].title, url: requestUrl + "sharing/rest/content/items/" + response.items[a].id });
                                } else {
                                    this.userContentInfo.myOrgGroups[value].items.push({ name: response.items[a].title, url: requestUrl + "sharing/rest/content/items/" + response.items[a].id });
                                }
                            }
                        }
            ...
            getOICFromFolder: function (value) {
                ...
                esriRequest(this.userContentInfo.user.userContentUrl + "/" + id, {
                    ...
                    if (response.items) {
                        ...
                        for (var a = 0; a < response.items.length; a++) {
                            if (response.items[a].type === "Oriented Imagery Catalog") {
                                this.userContentInfo.myFolders[value].items.push({ name: response.items[a].title, url: requestUrl + "sharing/rest/content/items/" + response.items[a].id });
                            }
                        }
                        var items = this.userContentInfo.myFolders[value].items;
                        items.sort(function (a, b) {
                ...
                esriRequest(requestUrl + "sharing/rest/content/groups/" + id, {
                    ...
                    if (response.items) {
                        ...
                        if (count) {
                            response.items.forEach(lang.hitch(this, function (item) {
                                if (item.type === "Oriented Imagery Catalog") {
                                    esriRequest(requestUrl + "sharing/rest/content/items/" + item.id + "/data", {
                                        query: {
                                            f: "json"
                                        },
                                        responseType: "json"
            ...
            checkInput: function () {
                ...
                if (input === "url")
                    ...
                    this.addOICFile();
                    else if (input === "agol" || input === "enterprise") {
            ...
            addImageOnMap: function (response) {
                ...
                var multipoint3 = this.createMultiPoint({type:"frustum", geometries: [response.image]});
                multipoint = multipoint.concat(multipoint2);
                multipoint = multipoint.concat(multipoint3);
                var image = response.image.clone();
                this.sceneView.map.ground.queryElevation(new Multipoint({points:multipoint, spatialReference: this.sceneView.spatialReference})).then(lang.hitch(this, function (res) {
                    for (var cv = 0; cv < res.geometry.points.length; cv++) {
                        if (cv < response.imageSourcePoints.length) {
                            response.imageSourcePoints[cv].z += res.geometry.points[cv][2];
                        } else {
            ...
            updateImageOnMap: function (response) {
                var multipoint = this.createMultiPoint({type:"frustum", geometries: [response.image]});
            var image = response.image.clone();
              this.sceneView.map.ground.queryElevation(new Multipoint({points:multipoint,spatialReference: this.sceneView.spatialReference})).then(lang.hitch(this, function (res) {
                    var cv =0;    
                            for (var dm = 2; dm < image.vertexAttributes.position.length; dm = dm + 3) {
            ...
            setCameraView: function (att) {
                ...
                if (att) {
                    ...
                    result.geometry.z += att.location.z;
                        ...
                        var pt = new Point({spatialReference: this.sceneView.spatialReference, 
                            x: result.geometry.x + (att.direction[0] * (distAssetToExposurePoint - distAssetToSceneCamera)),
                            y: result.geometry.y + (att.direction[1] * (distAssetToExposurePoint - distAssetToSceneCamera)),
                            z: result.geometry.z + (att.direction[2] * (distAssetToExposurePoint - distAssetToSceneCamera)/this.WMSF)
                                });
                                var fov = this.sceneView.camera.fov;
                            } else {
                    ...
                    this.currentImageCameraLocation = result.geometry.clone();   //kushagra change
                    var camera = this.sceneView.camera.clone();
                    camera.fov = cam.fov;
                    camera.heading = cam.heading; //camheading error fix
                   this.sceneView.camera = camera;
            ...
            zoomImageOnMap: function (scaleFactor, x, y) {
                var camera = this.sceneView.camera.clone();
                this.orientedViewer.zoomImageOnMap({
                ...
                for (var a = 0; a < layers.length; a++) {
                    if (layers[a].type === "feature") {
                        ...
                        this.vectorLayers[layers[a].id] = {
                            ...
                            editable: layers[a].editingEnabled && layers[a].capabilities && layers[a].capabilities.operations.supportsEditing,
            ...
            openImageInViewer: function () {
                if (this.selectedGraphicProperties && this.oiApiLoaded) {
                    ...
                    this.orientedViewer.searchImages(point.toJSON(), url, { maxDistance: 1000, mapSize: { w: this.sceneView.width, h: this.sceneView.height }, objectId: parseInt(this.selectedGraphicProperties.id), extent: this.sceneView.extent.toJSON(), camera:this.sceneView.camera });
            ...
            showAttributeWindow: function (layer, graphic, objectId) {
                ...
                for (var i = 0; i < layer.fields.length; i++) {
                    if (layer.fields[i].editable === true) {
                        if (layer.fields[i].name === 'ImgUrn' || layer.fields[i].name === 'ImgGeom') {
                            fieldInfos.push({ name: layer.fields[i].name, label: layer.fields[i].name, editable: false });
                        } else {
