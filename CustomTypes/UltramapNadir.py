...
def main(oicPath, oicParas, inputParams, defValues, log=None):
    try:
        ...
        eoFilename = oicParas['MetadataFile']
        preFix = oicParas["Prefix"]
        postFix = oicParas['Suffix']
        rasterDir = oicParas['RasterPath']
        rotatedCams = oicParas['RotatedCameras']
        averageZ = oicParas['AverageZ(meters)']
        nadirType = oicParas['Nadir Type(Panchromatic/MS)'] or 'panchromatic'
        if not averageZ:
            averageZ = 0
        else:
            try:
        ...
        cameraType = 'ultramap'
        productType = 'nadir'
        frame_camera.writeToFCUltramap(eoFilename, preFix, postFix, rasterDir, rotatedCams, cameraType, productType, oicPath, defValues, averageZ, nadirType)
        return("Successfully added list of images.")
    except Exception as e:
        if log:
            log.Message("Error in adding imagelist. {}".format(str(e)), log.const_critical_text)
        return ("Error in adding imagelist." + str(e))
